...
//        let vc = WebController.init("\(h5BaseUrl)/qx-floor-app/#/\(action!)")
        var httpStr = serverFloorJsUrl
        //serverGDJsUrl
        
        if let userType = getUser()?.userType {
            if userType == "LM" {
                httpStr = serverFloorJsUrl
            }else if userType == "GD"{
                httpStr = serverGDJsUrl
            }else if userType == "JS"{
                httpStr = serverJSJsUrl
            }
        }
        let vc = WebController.init("\(httpStr)/#/\(action!)")
